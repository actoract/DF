...
const CartDetails = ({id, text, image, uploadImage, details, type}) => {
    ...
    return (
        ...
        type === "header"?
            ...
            <Link to = {`/order/${id}`}><Row className = "CardDetails2 details">
                {id ? <Col md={4}> {id}</Col>: <div/>}
                {text ? text.map(item => 
                    item == false ? (<Col md={2}> <strong>NOT DELIVERED </strong> {} </Col>) : 
                    (<Col md={2}> {item} </Col>)) : <div/>
                }
            </Row>
            </Link>
            </Popover>
